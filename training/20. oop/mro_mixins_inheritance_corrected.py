...
class SuperHero(Person, FlyMixin, SuperJumpMixin):
    def __init__(self, name):
        super().__init__(name)  # Thanks to super, we initialize the name
        print(f"{self.name} can now fly and super jump!")  # Thanks for having extra powers!
...
class Villain(Person, SwimMixin):
    def __init__(self, name):
        super().__init__(name)  # Use Person's initialization
        print(f"{self.name} can swim, walk, and talk, but in a villainous way!")  # Watch out!

# Now let's add even more complexity with an AntiHero
# An AntiHero shares traits from both SuperHero and Villain, but no duplicate mixins.
...
# Create a SuperHero with flying and jumping abilities
...
villain.walk()  # Lex Luthor is walking
...
antihero.talk()  # Deadpool can talk
